...
class Renderer {
  ...
  private init() {
    ...
    Object.assign(program as object, {
  ...
  render(now: number) {
    ...
    gl.uniform2f((program as unknown as Record<string, WebGLUniformLocation>).resolution, canvas.width, canvas.height);
    gl.uniform1f((program as unknown as Record<string, WebGLUniformLocation>).time, now * 1e-3);
    gl.uniform3fv((program as unknown as Record<string, WebGLUniformLocation>).u_color, this.color);
...
export function SmokeBackground({ smokeColor = "#E63030" }: SmokeBackgroundProps) {
